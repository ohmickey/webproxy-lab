...
void doit(int connfd)
{
    int end_serverfd; /* end server file descripter */

    char buf[MAXLINE],method[MAXLINE],uri[MAXLINE],version[MAXLINE];
    /*store the request line arguments*/
    char endserver_http_header [MAXLINE]; // storage for end server header
    char hostname[MAXLINE],path[MAXLINE]; // storage for hostname, path
    int port;
    ...
    if(strcasecmp(method,"GET")){ // if same, return 0 (excpet lower || upper case)
