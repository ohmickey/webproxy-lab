...
void doit(int fd){  // Transaction 단위 처리, Tiny는 GET 메소드만 지원.
    ...
    Rio_readinitb(&rio, fd); // 식별자 fd 가 저장되어있는 주소를 rio 로 설정?
    Rio_readlineb(&rio, buf, MAXLINE); // rio 에 있는 text를 robustly 읽어와서 buf 에 쓴다.
    printf("Request headers:\n");
    printf("%s", buf); // 출력 퍼에 Request headers + buf(method, uri, version)
    ...
    read_requesthdrs(&rio); // request header   역할..?

    // Parse URI from GET request
    is_static = parse_uri(uri, filename, cgiargs); // 콘텐츠가 정적(1)/동적(0) 인지 나타내는 flag 생성
    if (stat(filename, &sbuf) < 0){ // filename의 Attribute을 가져와서 sbuf 에 저장.
